...
- Adopted setting: `[tool.mutmut] timeout_constant = 4.0` (near-zero-estimate floor budget ~60 s ≈ 78% margin over the measured healthy worst case of 33.8 s); cost is that genuine hangs are now reaped per-child within ≤60 s instead of ~45 s. The knob remains flagged "unstable" upstream; rationale + revisit condition documented in the `pyproject.toml` comment block. Final store snapshot `/tmp/mutmut-final-knob4-20260901-161010/`: {KILLED 8868, SURVIVED 5698, NO-TESTS 1601, TIMEOUT 2}, full keyset join with the pre-knob state, zero kills lost.
...
Upgrade path adopted and validated (monkeypatch-in-launcher alternative dropped as unnecessary): pin `"mutmut>=3.7.0,<4.0"`, migrate the two config keys, set `timeout_constant = 4.0` after measured A/B validation, keep launcher + ledger tooling unchanged (schema-compatible). Remaining follow-ups: regenerate baseline ledger v8 on the 16,161-key store before any further ratchet comparison, optionally post our measured repro data to boxed/mutmut#518, and product-triage `_projection_heartbeat`: its broad `except Exception` with no-delay retry means any persistent exception source becomes an unbounded event-loop hot spin in production too — the two remaining timeout verdicts are exactly this class manifesting.

## Implementation Guidance

- **Objectives**: keep mutation-gate numbers trustworthy (no false timeouts); track new mutator coverage into the baseline
- **Key Tasks**: regenerate baseline from `/tmp/mutmut-final-knob4-20260901-161010/`; re-run gate E2E (`TESTING=true .venv/bin/python scripts/mutmut_ledger.py gate --base HEAD`) after regeneration; decide #518 comment; product-triage `_projection_heartbeat` hot-spin risk
- **Dependencies**: none outstanding for the upgrade itself
- **Success Criteria (met)**: scoped + full runs on 3.7.0 complete rc=0 with zero spurious timeouts; the only remaining timeout verdicts are the two known infinite-loop mutants, each killed exactly at its own per-PID deadline under both tested constants (correct watchdog behavior); zero killed verdicts lost vs prior censuses on every join
