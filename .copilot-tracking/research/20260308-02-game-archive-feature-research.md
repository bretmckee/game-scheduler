...
- `GameStatus` (post-consolidation)
  - `ARCHIVED = "ARCHIVED"` is available
  - `is_valid_transition`: `COMPLETED → ARCHIVED` is valid; `CANCELLED → ARCHIVED` is not
...
### State Machine (post-consolidation)
