...
**Related Plan**: 20251218-game-creation-ui-consolidation.plan.md
...
- frontend/src/pages/**tests**/CreateGame.test.tsx - NEW FILE: Created comprehensive test suite with 8 tests covering loading state, server dropdown rendering with multiple guilds, auto-selection of single server with automatic template loading, template loading on server selection, warning display when no servers are available, template description display, GameForm rendering after template selection, and server selection change behavior with template refresh
...
- frontend/src/pages/**tests**/MyGames.test.tsx - Updated tests to expect new simplified /games/new routing, removed ServerSelectionDialog tests
