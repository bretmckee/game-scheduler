...
- frontend/src/api/calendarExport.ts - new file, `mintCalendarExportToken`/`buildCalendarExportUrl` stubs (throw `Error('... not yet implemented')`) (Task 6.1, RED)
- frontend/src/pages/**tests**/DownloadCalendar.test.tsx - rewritten: replaced the 6 `globalThis.fetch`-mocking tests with 5 `test.fails` cases mocking `apiClient.post` (mint success/navigate, 403, 404, generic-error, error-alert-close), each also asserting the mint call args; retained the unchanged loading-spinner test as a plain passing test (Task 6.1, RED)
- frontend/src/api/calendarExport.ts - implemented `mintCalendarExportToken` (`apiClient.post` to `/api/v1/export/game/{gameId}/token`, returns `response.data.token`) and `buildCalendarExportUrl` (returns `/api/v1/public/calendar/{token}.ics`) (Task 6.2, GREEN)
- frontend/src/pages/DownloadCalendar.tsx - `downloadCalendar` rewritten to call `mintCalendarExportToken`/`buildCalendarExportUrl` then set `window.location.href`; removed the `fetch`/`Blob`/`createObjectURL`/`a.click()` code and the now-unused `Time` import; error handling now reads `err.response?.status` instead of a `Response.status`, mapping 403/404/other to the same three messages as before (Task 6.2, GREEN)
- frontend/src/pages/**tests**/DownloadCalendar.test.tsx - removed `test.fails` markers from the 5 Task 6.1 tests (assertions unchanged); `beforeEach` now replaces `window.location` with a plain writable object (jsdom does not implement real navigation, so a direct `href` assignment on the native object is not observable) (Task 6.2, GREEN)
- frontend/src/components/**tests**/ExportButton.test.tsx - new file, 3 `test.fails` cases (mint success/navigate, 403 alert, generic alert) mocking `mintCalendarExportToken` (real `buildCalendarExportUrl` kept via `importOriginal`), plus a retained plain "renders the export button" test (Task 7.1, RED)
- frontend/src/pages/**tests**/GameDetails.calendar_export.test.tsx - new file, 2 `test.fails` cases (mint success/navigate, 403 alert) for `handleDownloadCalendar`, plus a retained plain "renders the Download calendar event button" test; mocks `apiClient` and `mintCalendarExportToken` (Task 7.1, RED)
- frontend/src/components/ExportButton.tsx - `downloadCalendar` rewritten to call `mintCalendarExportToken`/`buildCalendarExportUrl` then set `window.location.href`; removed the `axios.get(..., { responseType: 'blob' })` + Blob-download code and the `axios` import; error-presentation (`alert(errorMessage)`) unchanged (Task 7.2, GREEN)
- frontend/src/pages/GameDetails.tsx - `handleDownloadCalendar` rewritten the same way; removed the now-unused `axios` import, added `mintCalendarExportToken`/`buildCalendarExportUrl` import; error-presentation (`alert(errorMessage)`) unchanged (Task 7.2, GREEN)
- frontend/src/components/**tests**/ExportButton.test.tsx, frontend/src/pages/**tests**/GameDetails.calendar_export.test.tsx - removed `test.fails` markers from the Task 7.1 tests (assertions unchanged) (Task 7.2, GREEN)
