...
5. **RabbitMQ Channel Closure** - Both SyncEventPublisher and EventPublisher checked if channel was None but not if channel was closed, causing ChannelWrongStateError. Added `is_open`/`is_closed` checks and automatic reconnection logic
6. **Verbose Notification Messages** - Game reminder DMs displayed both Discord's automatic countdown and the notification interval (e.g., "starts in 28 minutes (in 50 minutes)"). Removed redundant interval suffix
