...
**Related Plan**: 20260116-default-complexity-thresholds-reduction.plan.md
...
- [tests/services/api/routes/test_games_helpers.py](tests/services/api/routes/test_games_helpers.py) - Unit tests for extracted helper functions (\_parse_update_form_data and \_process_image_upload); Phase 4: Added 8 unit tests for \_build_game_response extracted helpers (TestBuildGameResponseHelpers class) covering display data resolution, Discord API fetching, participant response building, and host response creation
...
- [services/api/routes/games.py](services/api/routes/games.py) - Extracted \_parse_update_form_data() and \_process_image_upload() helpers, refactored update_game() to reduce complexity from C:14 to A:5; Phase 4: Extracted \_resolve_display_data(), \_fetch_discord_names(), \_build_participant_responses(), and \_build_host_response() helpers, refactored \_build_game_response() to reduce cognitive complexity from Cog:16 to ≤15
...
- [services/api/services/display_names.py](services/api/services/display_names.py) - Extracted \_check_cache_for_users(), \_fetch_and_cache_display_names_avatars(), and \_create_fallback_user_data() helpers, refactored resolve_display_names_and_avatars() to reduce complexity from C:12/Cog:19 to below thresholds; Phase 4: Extracted \_check_cache_for_display_names(), \_fetch_and_cache_display_names(), and \_create_fallback_display_names() helpers, refactored resolve_display_names() to reduce cognitive complexity from Cog:20 to ≤15
...
- [services/bot/events/handlers.py](services/bot/events/handlers.py) - Phase 2: Extracted \_validate_game_for_reminder(), \_partition_and_filter_participants(), \_send_participant_reminders(), and \_send_host_reminder() helpers, refactored \_handle_game_reminder() to reduce cognitive complexity from Cog:23 to below threshold; Phase 3: Extracted \_fetch_join_notification_data(), \_is_participant_confirmed(), \_format_join_notification_message(), and \_send_join_notification_dm() helpers, refactored \_handle_join_notification() to reduce cognitive complexity from Cog:19 to Cog:8; Phase 4: Extracted \_fetch_game_for_refresh(), \_validate_channel_for_refresh(), \_fetch_message_for_refresh(), \_update_game_message_content(), and \_set_message_refresh_throttle() helpers, refactored \_refresh_game_message() to reduce cognitive complexity from Cog:18 to Cog:10 (44% reduction)
- [services/api/services/participant_resolver.py](services/api/services/participant_resolver.py) - Extracted \_resolve_discord_mention_format(), \_resolve_user_friendly_mention(), and \_create_placeholder_participant() helpers, refactored resolve_initial_participants() to reduce complexity from C:12/Cog:20 to B:7; Phase 4: Extracted \_process_single_participant_input() helper, refactored resolve_initial_participants() to reduce cognitive complexity from Cog:16 to ≤15
...
- [tests/services/api/services/test_display_names.py](tests/services/api/services/test_display_names.py) - Added 10 unit tests for extracted helper methods covering cache checking, Discord API fetching, and fallback data creation; Phase 4: Added 8 unit tests for resolve_display_names extracted helpers covering cache checking, fetching/caching, and fallback name generation
...
- [tests/services/api/services/test_participant_resolver.py](tests/services/api/services/test_participant_resolver.py) - Added 13 unit tests for extracted helper methods covering Discord mention format resolution, user-friendly mention search, and placeholder participant creation; Phase 4: Added 7 unit tests for \_process_single_participant_input helper covering Discord mentions, user-friendly mentions, placeholders, empty inputs, and error cases
...
- [x] **Task 4.3**: Refactored `_refresh_game_message` (services/bot/events/handlers.py:275-323) - Cog: 18→10
  - Extracted `_fetch_game_for_refresh()` for game data retrieval with validation
  - Extracted `_validate_channel_for_refresh()` for channel API validation and bot retrieval
  - Extracted `_fetch_message_for_refresh()` for Discord message fetching with error handling
  - Extracted `_update_game_message_content()` for message content/embed/view updates
  - Extracted `_set_message_refresh_throttle()` for Redis throttle key management
  - Added 11 unit tests in [tests/services/bot/events/test_handlers.py](tests/services/bot/events/test_handlers.py:2034-2243) covering all helper methods
  - Successfully reduced cognitive complexity from 18→10 (44% reduction)
  - All tests passing (89 total in file)
- [x] **Task 4.4**: Addressed remaining cognitive complexity violations (Cog:16-20)
  - **4.4.1**: Refactored `_build_game_response` (services/api/routes/games.py:618-729) - Cog: 16→≤15
    - Extracted `_resolve_display_data()` for display name/avatar resolution
    - Extracted `_fetch_discord_names()` for Discord API channel/guild name fetching
    - Extracted `_build_participant_responses()` for participant response list building
    - Extracted `_build_host_response()` for host participant response creation
    - Added 8 unit tests in [tests/services/api/routes/test_games_helpers.py](tests/services/api/routes/test_games_helpers.py:234-382) (TestBuildGameResponseHelpers class)
    - Successfully reduced cognitive complexity from 16→≤15
  - **4.4.2**: Refactored `resolve_display_names` (services/api/services/display_names.py:90-143) - Cog: 20→≤15
    - Extracted `_check_cache_for_display_names()` for cache checking with cached/uncached split
    - Extracted `_fetch_and_cache_display_names()` for Discord API fetching and caching
    - Extracted `_create_fallback_display_names()` for fallback name generation on errors
    - Added 8 unit tests in [tests/services/api/services/test_display_names.py](tests/services/api/services/test_display_names.py:649-853) covering all helper methods
    - Successfully reduced cognitive complexity from 20→≤15
  - **4.4.3**: Refactored `resolve_initial_participants` (services/api/services/participant_resolver.py:246-305) - Cog: 16→≤15
    - Extracted `_process_single_participant_input()` for single participant input processing
    - Handles Discord mention format, user-friendly @mentions, and placeholder strings
