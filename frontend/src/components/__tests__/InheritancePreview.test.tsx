...
describe('InheritancePreview', () => {
  it('renders label and value correctly', () => {
    render(<InheritancePreview label="Max Players" value={10} inherited={false} />);
  ...
  it('formats array values with commas', () => {
    render(<InheritancePreview label="Reminder Times" value={[60, 15, 5]} inherited={false} />);

    expect(screen.getByText('60, 15, 5')).toBeInTheDocument();
  });

  it('displays "Not set" for null values', () => {
    render(<InheritancePreview label="Rules" value={null} inherited={false} />);
