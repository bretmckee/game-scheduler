...
@app.task(name="services.scheduler.tasks.update_game_status.update_game_statuses")
def update_game_statuses():
    """Update game statuses based on scheduled times."""
...
def _mark_games_in_progress(db: Session, current_time: datetime.datetime) -> int:
    ...
    for game_session in games_to_start:
        if status_transitions.is_valid_transition(game_session.status, "IN_PROGRESS"):
            ...
            _publish_game_started_event(game_session)
            count += 1

    return count


def _publish_game_started_event(game_session: game.GameSession) -> None:
    """Publish game.started event to notify other services."""
    event_pub = SyncEventPublisher()
    ...
    try:
        ...
        event_pub.publish(event_wrapper)
