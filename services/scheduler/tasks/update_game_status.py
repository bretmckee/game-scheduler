...
async def _mark_games_in_progress(db: AsyncSession, current_time: datetime.datetime) -> int:
    ...
    for game_session in games_to_start:
        if status_transitions.is_valid_transition(game_session.status, "IN_PROGRESS"):
            ...
            await _publish_game_started_event(game_session)
            count += 1

    return count


async def _publish_game_started_event(game_session: game.GameSession) -> None:
