...
def wait_for_rabbitmq(
    rabbitmq_url: str, max_retries: int = 60, retry_delay: float = 1.0
) -> None:
    """
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    logger.debug(
                        f"RabbitMQ not ready (attempt {attempt}/{max_retries}): {e}"
                    )
...
def create_infrastructure(rabbitmq_url: str) -> None:
    ...
    with tracer.start_as_current_span("init.rabbitmq_infrastructure") as span:
        try:
            ...
            channel.exchange_declare(
                exchange=DLX_EXCHANGE, exchange_type="topic", durable=True
            )
