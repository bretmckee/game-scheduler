    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                print(
                    f"  RabbitMQ not ready (attempt {attempt}/{max_retries}), retrying..."
                )
                logger.debug(
                    f"RabbitMQ not ready (attempt {attempt}/{max_retries}), retrying..."
                )
                ...
                logger.error(
                    f"RabbitMQ failed to become ready after {max_retries} attempts"
                )
                sys.exit(1)
...
def create_infrastructure(rabbitmq_url: str) -> None:
    ...
    for dlq_name, routing_key in DLQ_BINDINGS:
        channel.queue_bind(
            exchange=DLX_EXCHANGE, queue=dlq_name, routing_key=routing_key
        )
...
def main() -> None:
    ...
    with tracer.start_as_current_span("init.rabbitmq") as span:
        ...
        if not rabbitmq_url:
            span.set_status(
                trace.Status(trace.StatusCode.ERROR, "RABBITMQ_URL not set")
            )
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize RabbitMQ infrastructure: {e}", exc_info=True
            )
            flush_telemetry()
            sys.exit(1)
