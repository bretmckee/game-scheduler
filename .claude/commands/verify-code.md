...
1. **Identify scope of changes:**
   - Use `git diff --name-only` (or `git status`) to identify all modified, added, and deleted files.
...
2. **Load applicable coding standards:**
   - `.github/instructions/coding-best-practices.instructions.md` for
     all code
   - `.github/instructions/python.instructions.md` for Python files
   - `.github/instructions/reactjs.instructions.md` for
...
- [ ] **Verify no compile/lint errors remain:**
  - Re-run the linters and type checkers above and confirm clean output
